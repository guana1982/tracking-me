...
describe('day-state helpers', () => {
  ...
  it('gives a free note half weight, so one sentence cannot outvote a day', () => {
    ...
    const day = new Map([
      ['rating:benessere', { label: 'Benessere', scores: [-0.11], details: [] }],
      ['rating:sonno', { label: 'Sonno', scores: [0.56], details: [] }],
      ['log:WORKOUT', { label: 'Note allenamento', scores: [scoreFromValence('NEGATIVE')], details: [] }],
    ]);
  ...
  it('averages each instrument before averaging the day, so nothing votes twice', () => {
    ...
    const twice = new Map([
      ['rating:benessere', { label: 'Benessere', scores: [-0.11, -0.11], details: [] }],
    ]);
