...
function buildReadme(from: string | undefined, to: string | undefined, tzOffset: number): string {
  const range =
    from || to
...
Estrai lo ZIP e allega tutti i file alla conversazione con il modello. Se il servizio usato
...
export function buildFoodAiPackage(csv: string, metadata: FoodAiExportMetadata): Uint8Array {
  return zipSync(
    {
      'diario-alimentare.csv': strToU8(csv),
      'LEGGIMI.md': strToU8(buildReadme(metadata.from, metadata.to, metadata.tzOffset)),
