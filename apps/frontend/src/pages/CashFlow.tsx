    ...
    <div className="sm:ml-16 space-y-4">
      ...
      <div className="card">
        ...
        <div className="mb-3 flex flex-col sm:flex-row gap-2">
          <input className="input sm:max-w-sm" value={newColumnLabel} onChange={(e) => setNewColumnLabel(e.target.value)} placeholder="Nuova colonna" />
          <button type="button" className="btn btn-primary" onClick={() => createColumn.mutate({ label: newColumnLabel.trim() }, { onSuccess: () => setNewColumnLabel('') })}>
            Aggiungi
          </button>
        </div>
            ...
            <div key={column.key} className="flex items-center justify-between rounded-lg border border-slate-200 px-3 py-2">
              <div className="flex items-center gap-2">
                {editingColumnKey === column.key ? (
                  <input className="input h-9 w-56" value={editingColumnLabel} onChange={(e) => setEditingColumnLabel(e.target.value)} />
                ) : (
              ...
              <div className="flex items-center gap-1">
                ...
                <button type="button" className="p-1.5 rounded hover:bg-slate-100" onClick={() => updateColumn.mutate({ key: column.key, data: { isActive: !column.isActive } })}>
                  {column.isActive ? <Eye className="w-4 h-4" /> : <EyeOff className="w-4 h-4" />}
                </button>
                {editingColumnKey === column.key ? (
                  <>
                    <button type="button" className="p-1.5 rounded text-emerald-600 hover:bg-emerald-50" onClick={() => updateColumn.mutate({ key: column.key, data: { label: editingColumnLabel.trim() } }, { onSuccess: () => setEditingColumnKey(null) })}><Check className="w-4 h-4" /></button>
                    <button type="button" className="p-1.5 rounded hover:bg-slate-100" onClick={() => setEditingColumnKey(null)}><X className="w-4 h-4" /></button>
                  </>
                ) : (
                  <button type="button" className="p-1.5 rounded text-sky-600 hover:bg-sky-50" onClick={() => { setEditingColumnKey(column.key); setEditingColumnLabel(column.label); }}><Pencil className="w-4 h-4" /></button>
                )}
                <button type="button" className="p-1.5 rounded text-red-600 hover:bg-red-50" onClick={() => deleteColumn.mutate(column.key)}><Trash2 className="w-4 h-4" /></button>
              </div>
            </div>
          ))}
        </div>
          ...
          {pieData.length === 0 ? (
            ...
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={pieData} dataKey="value" nameKey="label" cx="50%" cy="50%" innerRadius={56} outerRadius={88}>
                    {pieData.map((item) => (
      ...
      <div className="card">
        ...
        {rowsWithMetrics.length === 0 ? (
          ...
          <div className="overflow-auto">
            <table className="w-full text-sm" style={{ minWidth: `${800 + activeColumns.length * 150}px` }}>
              <thead className="text-xs uppercase tracking-wide text-slate-500">
                <tr>
                  <th className="text-left py-2 pr-3">Azioni</th>
                  <th className="text-left py-2 pr-3">Check</th>
                  <th className="text-left py-2 pr-3">Data</th>
                  {activeColumns.map((column) => (
                    <th key={column.key} className="text-right py-2 px-2">{column.label}</th>
                  ))}
                ...
                {rowsWithMetrics.map((row) => (
                  <tr key={row.id} className="border-t border-slate-100">
                    <td className="py-2 pr-3">
                      <div className="flex items-center gap-1">
                        <button type="button" className="p-1.5 rounded text-sky-600 hover:bg-sky-50" onClick={() => openEditModal(row)}><Pencil className="w-4 h-4" /></button>
                        <button type="button" className="p-1.5 rounded text-red-600 hover:bg-red-50" onClick={() => deleteCheck.mutate(row.id)}><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                    <td className="py-2 pr-3 font-medium">{row.checkLabel}</td>
                    <td className="py-2 pr-3">{formatDate(row.date)}</td>
                    {activeColumns.map((column) => (
                      <td key={column.key} className="py-2 px-2 text-right tabular-nums">{formatCurrency(getRowValue(row, column.key))}</td>
                    ))}
                    <td className="py-2 px-2 text-right tabular-nums font-semibold">{formatCurrency(row.total)}</td>
                    <td className="py-2 px-2 text-right tabular-nums">{row.diffTotal === null ? 'N/A' : formatCurrency(row.diffTotal)}</td>
